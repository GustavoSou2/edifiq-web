...
const BUYER_MAIN_NAV: NavItem[] = [
  ...
  { path: '/app/orders',     label: 'Pedidos',     icon: ICONS.orders,    badge: '3' },
...
const SUPPLIER_MAIN_NAV: NavItem[] = [
  ...
  { path: '/app/available-orders',   label: 'Pedidos Disponíveis',icon: ICONS.search,    badge: '8' },
      ...
      >
        ...
        @if (!sidebarCollapsed()) {
          <div class="sidebar__panel-badge" [class.sidebar__panel-badge--supplier]="rolePanel.isSupplier()">
            <span class="panel-badge__dot"></span>
      ...
      <main class="app-main">
        ...
        <header class="topbar">
          <div class="topbar__left">
            ...
            <div class="topbar__mobile-logo">
              <div class="logo-mark logo-mark--sm" [class.logo-mark--supplier]="rolePanel.isSupplier()">
            ...
            <div class="topbar__panel-info">
              <span class="topbar__panel-dot" [class.topbar__panel-dot--supplier]="rolePanel.isSupplier()"></span>
              <span class="topbar__panel-label">Painel {{ rolePanel.currentConfig().label }}</span>
            </div>
          </div>
          ...
          <div class="topbar__right">
            ...
            <button class="topbar-btn topbar-btn--hide-mobile" type="button" aria-label="Ajuda">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                   stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                <path [attr.d]="icons.help"/>
              </svg>
            </button>
          </div>
        </header>
        ...
        @if (showTransitionBanner()) {
          <div
            ...
            aria-live="polite"
          >
            <span class="panel-transition-banner__dot"></span>
            ...
            <button class="panel-transition-banner__close" type="button"
                    aria-label="Fechar" (click)="dismissBanner()">
    ...
    <nav class="bottom-nav" aria-label="Navegação principal mobile">
      @for (item of mainNav().slice(0, 4); track item.path) {
        <a
          ...
          [class.bottom-nav__item--supplier]="rolePanel.isSupplier()"
          [routerLink]="item.path"
          [attr.aria-label]="item.label"
          [attr.aria-current]="isActiveRoute(item.path) ? 'page' : null"
        >
          @if (item.badge) {
            <span class="bottom-nav__badge" aria-hidden="true">{{ item.badge }}</span>
          }
...
export class AppLayoutComponent {
  ...
  dismissBanner():    void { this.showTransitionBanner.set(false); }
