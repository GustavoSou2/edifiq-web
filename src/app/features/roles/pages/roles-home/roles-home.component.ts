...
@Component({
  ...
  template: `
    ...
    @if (isLoading()) {
      <div class="loading-state" aria-live="polite">Carregando perfis...</div>
    } @else {
...
export class RolesHomeComponent implements OnInit {
  ...
  openEdit(role: Role): void {
    ...
    this.form = { name: role.name, description: '', permissions: [...role.permissions] };
    this.formSubmitted.set(false);
    this.formError.set('');
    this.modalOpen.set(true);
  }
  ...
  togglePerm(key: string): void {
    ...
    this.form.permissions = perms.includes(key) ? perms.filter(p => p !== key) : [...perms, key];
  }

  selectAll(): void { this.form.permissions = ALL_PERMISSIONS.map(p => p.key); }
  clearAll():  void { this.form.permissions = []; }
  ...
  async saveRole(): Promise<void> {
    ...
    const payload: CreateRolePayload = { name: this.form.name.trim(), permissions: this.form.permissions };
    this.saving.set(true);
    try {
      if (this.editingId()) {
        const updated = await firstValueFrom(this.usersApi.updateRole(this.editingId()!, payload));
  ...
  confirmDelete(role: Role): void { this.deleteTarget.set(role); }
